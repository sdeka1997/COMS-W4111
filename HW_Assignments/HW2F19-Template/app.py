...
def resource_by_id(dbname, resource, primary_key):
    ...
    try:
        ...
        if request.method == 'GET':
            ...
            response = Response(json.dumps(result), status=200, content_type="application/json")
            return "HTTP: 410 - " + str(count-new) + " rows successfully deleted."
...
def get_resource(dbname, resource_name):
    ...
    try:
        ...
        if request.method == 'GET':
            result = r_table.find_by_template(template=context["query_params"], field_list=fields)
            ...
            response = Response(json.dumps(result), status=200, content_type="application/json")
            return "HTTP: 201 - " + str(new - count) + " rows successfully inserted."
